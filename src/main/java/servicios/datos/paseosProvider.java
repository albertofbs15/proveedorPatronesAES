 ...
 */
public class paseosProvider implements PersistenciaService {
    ...
    public List<DetalleServicio> consultarDisponibilidad(String destino, LocalDate fechaInicial, LocalDate fechaFinal) {
        ...
        List<DetalleServicio> list = Stream.iterate(fechaInicial.plusDays(1), d -> d.plusDays(1))
                .limit(ChronoUnit.DAYS.between(fechaInicial, fechaFinal))
                .map(date -> consultarDisponibilidad(destino, date).orElse(null)).filter(detalle -> detalle != null).collect(Collectors.toList());

        if (list.size() == fechaInicial.until(fechaFinal, ChronoUnit.DAYS) - 1) {
            return list;
        }
